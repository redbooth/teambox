  ...
  class Attachment
    ...
    def validate_size options #:nodoc:
      if file? && !options[:range].include?(size.to_i)
        validation_key = :attachment_file_size_invalid
        action_view = ::ActionController::Base.helpers
        interpolate = { }
        
        if options[:range].first == 0
          validation_key = :attachment_file_size_less_than
        else
        ...
        if options[:range].last.is_a?(Float) && options[:range].last.infinite?
          validation_key = :attachment_file_size_greater_than
        else
          interpolate[:max] = action_view.number_to_human_size(options[:range].last)
        end
        
        interpolate[:message] = options[:message] || validation_key
        interpolate
      end
    end

    def validate_presence options #:nodoc:
      { :message => options[:message] } unless file?
    end
    ...
    def validate_content_type options #:nodoc:
      ...
      unless original_filename.blank?
        unless valid_types.blank?
          ...
          { :message => options[:message] } unless valid_types.any?{|t| content_type.nil? || t === content_type }
        end
      end
    end
    ...
    def flush_errors #:nodoc:
      @errors.each do |error, message|
        [message].flatten.each { |m| instance.errors.add(name, m.delete(:message), m) }
      end
    end
