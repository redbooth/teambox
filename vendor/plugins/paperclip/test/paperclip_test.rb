...
class PaperclipTest < Test::Unit::TestCase
  ...
  context "An ActiveRecord model with an 'avatar' attachment" do
    ...
    context "with size validation and less_than 10240 option" do
      context "and assigned an invalid file" do
        ...
        should "have a file size max error message" do
          assert_match /file size must be less than 10 KB/, @dummy.errors.on(:avatar)
        end
      end
    end
    
    context "with size validation and greater_than 10240 option" do
      context "and assigned an invalid file" do
        setup do
          Dummy.send(:"validates_attachment_size", :avatar, :greater_than => 10240)
          @dummy = Dummy.new
          @dummy.avatar &&= File.open(File.join(FIXTURES_DIR, "5k.png"), "rb")
          @dummy.valid?
        end
        
        should "have a file size min error message" do
          assert_match /file size must be greater than 10 KB/, @dummy.errors.on(:avatar)
        end
      end
    end
    
    context "with size validation and less_than 10240 and greater_than 8096 option" do
      context "and assigned an invalid file" do
        setup do
          Dummy.send(:"validates_attachment_size", :avatar, :less_than => 10240, :greater_than => 8096)
          @dummy = Dummy.new
          @dummy.avatar &&= File.open(File.join(FIXTURES_DIR, "5k.png"), "rb")
          @dummy.valid?
        end
        ...
        should "have a file size min/max error message" do
          assert_match /file size must be between 7.9 KB and 10 KB/, @dummy.errors.on(:avatar)
        end
      end
    end
    
    context "with presence validation and assigned an invalid file" do
      context "and custom string message option" do
        setup do
          Dummy.send(:"validates_attachment_presence", :avatar, :message => 'uh oh, no no')
          @dummy = Dummy.new
          @dummy.valid?
        end
        
        should "have a custom file presence error message" do
          assert_match /uh oh, no no/, @dummy.errors.on(:avatar)
        end
      end
      
      context "and custom symbol message option" do
        setup do
          I18n.backend.store_translations :en, { 
            :activerecord => { 
              :errors => { 
                :messages => { 
                  :uhoh => 'uh oh, no no' }}}}
          Dummy.send(:"validates_attachment_presence", :avatar, :message => :uhoh)
          @dummy = Dummy.new
          @dummy.valid?
        end
  ...
  context 'i18n support' do
    should 'add default en.yml locale file into the load path' do
      assert_contains(I18n.load_path.inspect, /lib\/paperclip\/locales\/en.yml/)
    end
    
    should "be able to translate attachment_presence" do
      assert_equal('must be set', I18n.t('activerecord.errors.messages.attachment_presence'))
    end
    
    should "call I18n.translate for error translation" do
      Dummy.send(:"validates_attachment_presence", :avatar)
      @dummy = Dummy.new
      
      # once for model, once for attribute translation, once for error
      I18n.expects(:translate).twice
      I18n.expects(:translate).with(:"models.dummy.attributes.avatar.attachment_presence", 
                                    {:model => nil, 
                                     :default => [:"models.dummy.attachment_presence", :"messages.attachment_presence"], 
                                     :value => @dummy.avatar, 
                                     :attribute => nil, 
                                     :scope => [:activerecord, :errors]}).returns('must be set')
      
      @dummy.valid?
      
      assert_equal(@dummy.errors.size, 1)
      assert_contains(@dummy.errors.on(:avatar), /must be set/)
    end
  end
end
