  ...
  module Model
    ...
    module ClassMethods
      ...
      def acts_as_fleximage(options = {})
        ...
        dsl_accessor :use_creation_date_based_directories, :default => false
        ...
        # Allow only images to be saved
        dsl_accessor :only_images, :default => true
        ...
        # Only images message
        #dsl_accessor :invalid_image_message, :default => 'was not a readable image'
        def self.only_images_message(str = nil)
          if str.nil?
            if @invalid_image_message
              @only_images_message
            else
              translate_error_message("only_images", "must be an image")
            end
          else
            @invalid_image_message = str
          end
    ...
    module InstanceMethods
      ...
      #   photo.save
      def file_string=(data)
        self.file = StringIO.new(data)
      end
      ...
      def file_base64=(data)
        self.file_string = Base64.decode64(data)
      end
      ...
      def output_image(options = {}) #:nodoc:
        if @output_image.respond_to?(:to_blob)
          @output_image.to_blob
        elsif @output_image.respond_to?(:read)
          @output_image.rewind
          @output_image.read
        end
      ensure
        @output_image.dispose! if is_image?
        GC.start
      end
      ...
      def validate_image #:nodoc:
        ...
        if !@uploaded_image and self.class.require_file
      ...
      private
        ...
        def post_save
          ...
          if @uploaded_image
            @uploaded_image.dispose! if is_image?
            GC.start
          end
        end
        ...
        def perform_preprocess_operation
          if is_image? and self.class.preprocess_image_operation
        ...
        # Save magic file columns (filename, content_type)
        def set_file_attributes(file = nil)
          if file
        ...
        def set_magic_attributes(file = nil)
          if is_image? and @uploaded_image and file
        ...
        # returns the content type of the file is it is an image
